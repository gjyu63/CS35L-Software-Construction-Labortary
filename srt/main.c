...
//Have to be declared as Global Variables, as they are used by multiThreadFunction.
int nthreads;
scene_t scene;
...
void *myFunction(void *arg){
    ...
    for( int px=*t; px<width; px=px+nthreads)
    {
...
int main( int argc, char **argv ){
    ...
    for(r=0; r<nthreads; r++){
        parameter[r] = r;
        if(pthread_create(&tid[r], NULL, myFunction, &parameter[r])) {
